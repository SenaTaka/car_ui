...
## 2026/07/20 13:35
- アプリアイコンを OBD2 コネクタ(16ピン台形・白グリフ+青グラデ)に刷新。単一1024構成へ移行し、iOS 18+ の dark/tinted バリアントも追加(旧アイコンはシステムのダーク変換で崩れていた)。シミュレータのホーム画面でライト/ダーク両方確認。
...
## 2026/08/02 19:40
- フィードバック 5 件対応(fde2c8e / e58eac8 / 多言語コミット):
  - 重すぎる原因究明: 主因はデモモードの毎秒 84 回 objectWillChange(19 PID 個別代入×4 Hz)。ティック毎 1 回の一括反映に修正。実接続もサイクル毎 2 回に集約。チャート 240 点間引き・コンター区間メモ化・プレビュー Map 再構築 60 点毎化も実施。
  - 保存データ復元を既定 30 分に制限(設定: 読み込まない/30分/2時間/すべて、キー data.restoreWindowMinutes)。テスト追加。
  - デモモード入口を接続シート→設定(その他タブ)へ移動。ダッシュボード空状態とオンボーディングの導線は残置。
...
## 2026/08/02 20:10
- UI フィードバック 3 件: チャートのプリセットを上部パネルへ / ダッシュボード編集のプリセットを最上段へ / その他タブの言語・保存データピッカーをラベル明示行に変更(メニュー式で項目名が見えなかった)。
- 検証: ビルド+テスト全パス。push 済み(1.0.2 トレインで TestFlight へ)。
